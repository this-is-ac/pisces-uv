...
class Coeffs(nn.Module):
    ...
    def forward(self, lowres_input):
        ...
        for layer in self.global_features_conv:
            x = layer(x)
        x = x.view(bs, -1)
        # print(f'global features: {x.shape}')

        for layer in self.global_features_fc:
            x = layer(x)
        global_features = x
        # print(f'global features fc: {global_features.shape}')
        ...
        # print(fusion_grid.shape, fusion_global.shape)
...
class HDRPointwiseNN_depthv2(nn.Module):

    def __init__(self, params):
        super(HDRPointwiseNN_depthv2, self).__init__()
