function [D, D_map] = FADE(I)
    ...
    %% Fog aware statistical feature extraction     
        % f1        
            MSCN_var                = reshape(var(im2col(MSCN, [ps ps], 'distinct'), "omitnan"),[row col]/ps);        
        % f2,f3                    
            ...
            MSCN_V_pair_L_var       = reshape(var(MSCN_V_pair_col_temp1, "omitnan"),[row col]/ps);          
            MSCN_V_pair_R_var       = reshape(var(MSCN_V_pair_col_temp2, "omitnan"),[row col]/ps);          
        % f4        
    ...
    %% MVG model distance                 
        ...
        % test param for each patch                
            ...
            cov_fog_param_patch     = var(feat', "omitnan")';
        ...
        % foggy level computation
            ...
            Df                      = mean(distance_patch, 'omitnan');   % Mean_distance_patch
        ...
        % fog-free level computation
            distance_patch          = sqrt(cell2mat(cellfun(@mtimes,cellfun(@mrdivide,mu_cell,cov_cell, 'UniformOutput',0),mu_transpose_cell', 'UniformOutput',0)));     
            Dff                     = mean(distance_patch, 'omitnan');   % Mean_distance_patch                
