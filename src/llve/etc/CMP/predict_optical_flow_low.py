...
filepaths = []
for f in folders:
    filepaths.extend(glob(os.path.join(rootdir, 'gt', f, '*.png')))
...
flowimg_path = [fp.replace('/gt/','/flow_img/')[:-4]+'.png' for fp in filepaths]


for i in range(len(filepaths)):
    # input an image and do some transform
    image = Image.open(filepaths[i]).convert('RGB').resize((256, 256))
    size = image.size
