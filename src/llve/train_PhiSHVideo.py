...
from dataset import HDRDataset_depth, UWDataset_depth
...
parser.add_argument('--video-data-path', default='./uw_video', type=str, help='path to the video dataset')
...
video_train_set = UWDataset_depth(params['video_data_path'], params=params, split='all')
video_train_loader = DataLoader(video_train_set, batch_size=args.bs, shuffle=True)
...
criterionPercep = PerceptualLoss()
...
for e in range(params['epochs']):
    ...
    if e % K == 0:
        ...
        for i, (low, full, target, depth, flow_low, flow_full) in enumerate(train_loader):
            ...
            loss = criterion(pred, target) + 0.1 * cos_loss(pred, target) + 0.2 * tv_loss(full, res, 1)
        ...
        for i, (low, full, depth, flow_low, flow_full) in enumerate(video_train_loader):

            low, full, depth, flow_low, flow_full = low.to(device), full.to(device), depth.to(device), flow_low.to(device), flow_full.to(device)
            optimizer.zero_grad()
            ...
            loss = criterionPercep(pred, full) * 0.1
            ...
            if (count+1) % params['log_interval'] == 0:
                tloss = total_loss.item()
                print('Epoch: {0} [{1}/{2}]\t loss={tloss:.5f} l1={Loss1:.5f} l2={Loss2:.5f} '.format(e, (count + 1)%len(train_loader), len(video_train_loader), tloss=tloss, Loss1=loss.item(), Loss2=loss_t.item()))
...
# python train_PhiSHVideo.py --weight 20 --bs 4 --log PHiSHVideo_02 --epochs 50 --test-out /home/aditya/llve/StableLLVE/results/PHiSHVideo_02 --resume True --ckpt-path /home/aditya/llve/StableLLVE/logs/PHiSHVideo_01_K10/checkpoint-49.pth
# python train_PhiSHVideo.py --weight 0 --bs 16 --log short_test2 --resume True --ckpt-path /home/aditya/llve/StableLLVE/logs/short_test/checkpoint-0.pth